...
const STEPS: Array<{ id: ApplicationStatus; label: string }> = [
  { id: 'new',         label: 'Submitted' },
  { id: 'reviewing',   label: 'Under Review' },
  { id: 'shortlisted', label: 'Shortlisted' },
  { id: 'accepted',    label: 'Awarded' },
];
  ...
  if (currentStatus === 'rejected') {
    return (
      <div className="flex items-center gap-3 text-destructive p-4 bg-destructive/10 rounded-lg border border-destructive/20">
        ...
        <div>
          <p className="font-semibold text-sm">Application Not Selected</p>
          <p className="text-xs text-destructive/80 mt-0.5">Review pivot recommendations below for similar opportunities.</p>
        </div>
      </div>
    );
  }
              ...
              <div className="relative flex items-center justify-center">
                {isCompleted && (
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className="w-8 h-8 rounded-full bg-emerald-500 flex items-center justify-center text-white shadow-sm z-10"
                  >
