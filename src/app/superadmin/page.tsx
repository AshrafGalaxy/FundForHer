...
import { Loader2, ShieldCheck, XCircle, ExternalLink, ShieldAlert, Flag, Headset } from 'lucide-react';
import type { ProviderProfile } from '@/server/db/user-data';
import Link from 'next/link';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { ModerationQueue } from '@/components/superadmin/ModerationQueue';
import { SupportDesk } from '@/components/superadmin/SupportDesk';

const SUPERADMIN_EMAIL = 'karanbainade02@gmail.com';

function KycVerificationTab({ db }: { db: any }) {
...
export default function SuperAdminDashboard() {
    ...
    if (!user || user.email !== SUPERADMIN_EMAIL) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-secondary/30 p-4 text-center animate-in fade-in zoom-in duration-300">
        ...
        <div className="container mx-auto px-4 py-12 max-w-6xl animate-in fade-in duration-500">
            <header className="mb-10 flex flex-col items-center text-center">
                ...
                <p className="text-lg text-muted-foreground mt-2 max-w-2xl">
                    Executive oversight panel for platform integrity, moderation, and user support operations.
                </p>
            </header>

            <Tabs defaultValue="kyc" className="w-full">
                <TabsList className="mb-8 bg-muted/50 p-1 w-full flex justify-start h-auto border">
                    <TabsTrigger value="kyc" className="text-sm py-2 px-6 rounded-md data-[state=active]:shadow-sm">
                        <ShieldCheck className="w-4 h-4 mr-2" /> KYC Verification
                    </TabsTrigger>
                    <TabsTrigger value="moderation" className="text-sm py-2 px-6 rounded-md data-[state=active]:shadow-sm">
                        <Flag className="w-4 h-4 mr-2" /> Moderation Queue
                    </TabsTrigger>
                    <TabsTrigger value="support" className="text-sm py-2 px-6 rounded-md data-[state=active]:shadow-sm">
                        <Headset className="w-4 h-4 mr-2" /> Support Desk
                    </TabsTrigger>
                </TabsList>

                <TabsContent value="kyc" className="space-y-4">
                    <KycVerificationTab db={db} />
                </TabsContent>
